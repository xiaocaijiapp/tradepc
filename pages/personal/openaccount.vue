  ...
  <div class="pay-state-container">
    <div class="content">
      <div class="pay-state">
        ...
        <span v-if="state == 'true'" class="state-text">开户成功</span>
        <span v-else-if="state == 'timeout'" class="state-text">开户超时</span>
        <span v-else class="state-text">开户失败</span>
        <div v-if="state == 'true'" class="check-order-state" @click="$router.push('/personal')">
          个人中心
        </div>
        <div v-else class="check-order-state" @click="$router.push('/personal')">
          个人中心
        </div>
      </div>
    </div>
